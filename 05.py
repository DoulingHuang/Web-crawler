...
for j, p in zip(json_url, url_para):
    ...
    print(a)
    # 工作內容
    job_desciption = '工作內容:' + json_data['data']['jobDetail']['jobDescription']
    # 職務類別
    ...
    full_content = job_desciption + \
                   + job_category_str + job_salary + job_attr + job_loc + \
                   job_resp + job_btrip + job_peri + job_vac + job_star + job_need
    # print(full_content)
    # 接受身分
    ...
    # 聯絡方式
    contact = '聯絡方式: ' + json_data['data']['contact']['hrName'] + json_data['data']['contact'][
        'email'] + json_data['data']['contact']['phone']
    full_require = require_role + require_workExp + require_edu + require_maj  + require_lan +  \
                   require_specialty + require_skill + require_other
    # print(full_require)
